...
class ModelCatalogProduct extends Model {
	...
	public function getProductsTemp($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			...
			if (!empty($data['filter_filter'])) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "product_filter pf ON (p2c.product_id = pf.product_id) LEFT JOIN " . DB_PREFIX . "product_temp pt ON (pf.product_id = pt.parent_id)";
			} else {
				$sql .= " LEFT JOIN " . DB_PREFIX . "product_temp pt ON (p2c.product_id = pt.parent_id)";
			}
		} else {
			$sql .= " FROM " . DB_PREFIX . "product_temp pt";
		}

		$sql .= " WHERE pt.status = '1' AND pt.date_available <= NOW() ";
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_name'])) {
				...
				foreach ($words as $word) {
					$implode[] = "pt.name LIKE '%" . $this->db->escape($word) . "%'";
				}
				...
				if (!empty($data['filter_description'])) {
					$sql .= " OR pt.name LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
				}
			}
			...
			if (!empty($data['filter_name'])) {
				$sql .= " OR LCASE(pt.model) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
				$sql .= " OR LCASE(pt.sku) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
		...
		if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
			if ($data['sort'] == 'pt.name' || $data['sort'] == 'pt.model') {
				$sql .= " ORDER BY LCASE(" . $data['sort'] . ")";
			} elseif ($data['sort'] == 'pt.price') {
				$sql .= " ORDER BY (CASE WHEN special IS NOT NULL THEN special WHEN discount IS NOT NULL THEN discount ELSE pt.price END)";
			} else {
				$sql .= " ORDER BY " . $data['sort'];
			}
		} else {
		...
		if (isset($data['order']) && ($data['order'] == 'DESC')) {
			$sql .= " DESC, LCASE(pt.name) DESC";
		} else {
			$sql .= " ASC, LCASE(pt.name) ASC";
		}
		...
		foreach ($query->rows as $result) {
			$product_data[$result['product_id']] =  array(
				...
				'option_value_ids' => $result['option_value_ids'],
	...
	public function getProductRelated($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY pr.product_id DESC ");
	...
	public function getTotalProductsTemp($data = array()) {
		$sql = "SELECT COUNT(DISTINCT pt.product_id) AS total";
