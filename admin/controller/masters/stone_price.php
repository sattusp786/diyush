        ...
        $results     = $this->model_masters_stone_price->getStonePriceExport($data);		
			...
			if($results){
				foreach ($results as $result) {
					...
					$stone_price .= $this->db->escape($result['stone_price_id']) . "," . $this->db->escape($result['stone']) . "," . $this->db->escape($result['shape']) . "," . $this->db->escape($result['crt_from']) . "," . $this->db->escape($result['crt_to']) . "," . $this->db->escape($result['weight']) . "," . $this->db->escape($result['clarity']) . "," . $this->db->escape($result['color']) . "," . $this->db->escape($result['lab']) . "," . $this->db->escape($result['cut']) . "," . $this->db->escape($result['polish']) . "," . $this->db->escape($result['symmetry']) . "," . $this->db->escape($result['fluorescence']) . "," . $this->db->escape($result['intensity']) . "," . $this->db->escape($result['carat_price']) . "," . $this->db->escape($result['total_price']) . "," . $this->db->escape($result['sprice']) . "," . $this->db->escape($result['mprice']) . "\n";
				}
			}
